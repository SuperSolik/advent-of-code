...
func checkDesign(design string, matcher map[byte][]string) bool {
	if possible, cached := cache[design]; cached {
		return possible
	}
	...
	if patterns, ok := matcher[design[0]]; ok {
		for _, p := range patterns {
			pSize := len(p)
			if pSize <= targetSize && strings.HasPrefix(design, p) && checkDesign(design[pSize:], matcher) {
				cache[design[pSize:]] = true
				return true
			}
		}
	}
...
func main() {
	...
	for i, d := range designs {
		possible := checkDesign(d, m)
		fmt.Printf("%d: %s, possible = %t\n", i, d, possible)
		if possible {
			result += 1
		}
	}
